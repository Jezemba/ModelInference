#!/usr/bin/env python3
"""
Llama Quant VQA Benchmark - HuggingFace Router API Version
-----------------------------------------------------------
This version uses HuggingFace's Router API with the quantized Llama-4-Maverick model.
...
def initialize_hf_client():
    """
    Initialize HuggingFace Router API client using OpenAI SDK.
    ...
    if not api_key:
        raise ValueError("HF_TOKEN environment variable not set. Please set it with your HuggingFace token.")

    print("Initializing HuggingFace Router API client...")
    client = OpenAI(
        base_url="https://router.huggingface.co/v1",
        api_key=api_key,
    )
    print("HuggingFace Router client initialized successfully")
    return client
...
def prepare_hf_messages(example, media_type, frames=None):
    """
    Prepare message format for HuggingFace Router API with structured output.
    ...
    Returns:
        messages list in OpenAI API format
    """
...
def run_inference_single(client, example, model_name, num_video_frames=8):
    """
    Run inference on a single example using HuggingFace Router API.

    Args:
        client: OpenAI client configured for HuggingFace Router
        example: Single example from dataset
    ...
    try:
        ...
        # Prepare messages
        messages = prepare_hf_messages(example, media_type, frames)
...
def run_benchmark(dataset_name, output_file='results_quant.csv', checkpoint_file='checkpoint_quant.pkl',
                  model_name="meta-llama/Llama-4-Maverick-17B-128E-Instruct-FP8:together",
                  num_video_frames=8, media_type="all", num_workers=1):
    """
    Run benchmark evaluation on the entire dataset with checkpointing using HuggingFace Router API.
    ...
    print(f"\n🚀 Starting processing with HuggingFace Router API...")
    ...
    def process_example(idx):
        ...
        try:
            # Run inference via HuggingFace Router API
    ...
    # Print summary
    print("\n" + "="*80)
    print("BENCHMARK RESULTS (HuggingFace Router API - Llama Quant)")
    print("="*80)
    print(f"\nModel: {model_name}")
...
if __name__ == "__main__":
    ...
    OUTPUT_FILE = "llama_quant_benchmark_results.csv"
    CHECKPOINT_FILE = "llama_quant_checkpoint.pkl"

    # HuggingFace Router Model - Quantized Llama-4-Maverick
    MODEL_NAME = "meta-llama/Llama-4-Maverick-17B-128E-Instruct-FP8:together"
    ...
    print("\n" + "="*80)
    print("IMPORTANT: Set your HuggingFace token before running:")
