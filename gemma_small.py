...
def load_benchmark_dataset(dataset_name, split='test', filter_media_type=None):
    """
    ...
    Args:
        ...
        filter_media_type: 'video', 'image', or None for all (default: None)
    ...
    # FILTER: Filter by media type without triggering video decoding
    if filter_media_type in ['video', 'image']:
        print(f"Filtering for {filter_media_type} examples only...")
        # Use select with indices to avoid decoding during filter
        media_indices = []
        for idx in tqdm(range(len(dataset)), desc=f"Finding {filter_media_type}s"):
            try:
        ...
        print(f"{filter_media_type.capitalize()} examples after filtering: {len(dataset)}")
    
    return dataset

def initialize_model(model_name="google/gemma-4b-it"):
    """
    Initialize Gemma 4b model and processor.
...
def prepare_messages(example, media_type, frames=None):
    ...
    if media_type == 'image':
        messages = [
            {
                "role": "system",
                "content": [{"type": "text", "text": "You are a helpful assistant for visual question answering."}]
            },
            {
                "role": "user",
                "content": [
...
def run_benchmark(dataset_name, output_file='results.csv', checkpoint_file='checkpoint.pkl',
                  model_name="google/gemma-3-4b-it", num_video_frames=32, filter_media_type=None):
    """
    ...
    Args:
        ...
        filter_media_type: Only process examples of this media type ('video', 'image', or None for all)
    
    Returns:
        Dictionary with evaluation metrics
    """
    # Load dataset
    dataset = load_benchmark_dataset(dataset_name, filter_media_type=filter_media_type)
    ...
    # Process one by one
    for idx in tqdm(unprocessed_indices, desc=f"Processing {filter_media_type or 'examples'}"):
        try:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Gemma Video Benchmark")
    parser.add_argument("--dataset", type=str, default="JessicaE/OpenSeeSimE-Structural", help="HuggingFace dataset name")
    parser.add_argument("--model", type=str, default="google/gemma-3-4b-it", help="Model name or path")
    parser.add_argument("--output", type=str, default="gemma3_4b_benchmark_results_video.csv", help="Output CSV file path")
    parser.add_argument("--checkpoint", type=str, default="gemma3_4b_checkpoint_video.pkl", help="Checkpoint file path")
    ...
    # Convert 'all' to None for filter_media_type
    filter_media_type = None if args.media_type == 'all' else args.media_type
