...
def load_benchmark_dataset(dataset_name, split='test', filter_media_type='video'):
    """
    Load the private benchmark dataset from HuggingFace, filtered for videos only.
    
    Args:
        dataset_name: Name of the dataset (e.g., 'JessicaE/OpenSeeSimE-Structural')
        split: Dataset split to load (default: 'test')
        filter_media_type: 'video' to filter for video examples only, or None for all
    ...
    if filter_media_type == 'video':
        print("Filtering for videos only...")
        # Use select with indices to avoid decoding during filter
        video_indices = []
        for idx in tqdm(range(len(dataset)), desc="Finding videos"):
            try:
                # Access the raw string field without decoding video
        ...
        print(f"Video examples after filtering: {len(dataset)}")
...
def run_inference_single(model, tokenizer, example):
    ...
    try:
        ...
        try:
            # Handle video (we only process videos in this script)
            try:
...
def run_benchmark(dataset_name, output_file='results.csv', checkpoint_file='checkpoint.pkl',
    ...
    """
    # Load dataset (filtering for videos only)
    dataset = load_benchmark_dataset(dataset_name, filter_media_type='video')
    
    # Check if we have any videos to process
    if len(dataset) == 0:
        print("No video examples found in the dataset. Exiting.")
        return None
    
    # Limit examples if requested
    if max_examples is not None and max_examples > 0 and max_examples < len(dataset):
        print(f"Limiting to first {max_examples} video examples")
    ...
    print(f"\n🚀 Starting processing...")
    print(f"   Total video examples: {len(dataset)}")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="InternVL Benchmark (Videos Only)")
    ...
    parser.add_argument("--output", type=str, default="internvl_videos_results.csv", 
                      help="Output CSV file path")
    parser.add_argument("--checkpoint", type=str, default="internvl_videos_checkpoint.pkl", 
...
if __name__ == "__main__":
    ...
    OUTPUT_FILE = "internvl3_5_1b_videos_only_results.csv"
    CHECKPOINT_FILE = "internvl3_5_1b_videos_only_checkpoint.pkl"
    MODEL_NAME = "OpenGVLab/InternVL3_5-1B-Instruct"
    MAX_EXAMPLES = 0  # Set to a positive number to limit examples, 0 for all
    
    # If running with arguments, use argparse
    if len(sys.argv) > 1:
        exit(main())
    else:
        # Otherwise use the predefined configuration
        summary = run_benchmark(
            DATASET_NAME, 
            OUTPUT_FILE, 
